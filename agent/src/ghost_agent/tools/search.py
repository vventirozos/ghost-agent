...
async def tool_fact_check(statement: str, http_client, tool_definitions, deep_research_callable: Callable, web_search_callable: Callable):
    pretty_log("Fact Check", statement, icon=Icons.STOP)
    
    allowed_names = ["deep_research", "web_search"]
    restricted_tools = [t for t in tool_definitions if t["function"]["name"] in allowed_names]
    
    messages = [
        {"role": "system", "content": "### ROLE: DEEP FORENSIC VERIFIER\nVerify this claim using the available tools.\n- Use `web_search` for simple facts (dates, versions, names).\n- Use `deep_research` for complex topics or if `web_search` is insufficient."},
    ...
    try:
        ...
        if func_name == "deep_research":
            ...
            research_result = await web_search_callable(**func_args)
        else:
